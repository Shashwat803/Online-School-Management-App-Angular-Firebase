...
export class VoucherComponent implements OnInit, OnDestroy {
  ...
  getData(formData?) {
    ...
    this.querySubscription = this._backendService.getDocs('VOUCHER', formData).subscribe((res) => {
  ...
  setData(formData) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.setDoc('VOUCHER', formData).then(res => {
      if (res) {
  ...
  updateData(formData) {
    this.dataLoading = true;
    this.querySubscription = this._backendService.updateDoc('VOUCHER', formData._id, formData).then(res => {
      if (res) {
        this.savedChanges = true;
        this.error = false;
        this.errorMessage = "";
  ...
  getDoc(docId) {
    this.dataLoading = true;
    this.data$ = this._backendService.getDoc('VOUCHER', docId).subscribe(res => {
      if (res) {
        this.data$ = res;
        this.editDataForm = this._fb.group({
  ...
  deleteDoc(docId) {
    if (confirm("Are you sure want to delete this record ?")) {
      this.dataLoading = true;
      this._backendService.deleteDoc('VOUCHER', docId).then(res => {
        if (res) {
          this.error = false;
          this.errorMessage = "";
          this.dataLoading = false;
